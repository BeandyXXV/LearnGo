...
func Data_type() {
	...
	*/
	...
	println("复数部分：")
	println(comp, comp_easy)

	// 对于一个复数，可以使用 real 和 imag 方法来获取它的实部和虚部
	println("实部：", real(comp), "\t", "虚部：", imag(comp))
	...
	*/
	...
	c := "b"
	println("a" <= c && c <= "z" || "A" <= c && c <= "Z" || "0" <= c && c <= "9")
	...
	*/
	...
	println(str_01 == str_02, len(str_01), len(str_02), len(str))
